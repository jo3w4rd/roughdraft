...
LocalToWorld (float4x4) represents the transform from local space to world space. It is the canonical representation and is the only component that can be relied upon to communicate local space among systems. 

- Some DOTS features may rely on the existance of LocalToWorld in order to function. 
...
If any combination of Translation (float3), Rotation (quaternion), or Scale (float) components are present along with a LocalToWorld component, a transform system combines those components and writes to LocalToWorld. 

Concretely, each of these component combinations writes to LocalToWorld as:
...
For example, if the following components are present...
...
Cases where ScalePivot is specified without either of Scale, NonUnfiformScale have no additional effect have no additional affect on CompositeScale.
...
```
----------------------------------
Hierarchical Transforms (Advanced)
----------------------------------
...
Parent LocalToWorld is multiplied with the Child LocalToWorld, which includes any scaling. However, if it removing Parent scale is preferred (AKA Scale Compensate), ParentScaleInverse is available for that purpose.
...
Cases where ScalePivot is specified without either of Scale, NonUnfiformScale have no additional affect on CompositeScale.
...
m_Group in UserTransformSystem will only match the explictly mentioned components.
