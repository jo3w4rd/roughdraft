...
void setup(){
  ...
  leap.config().setFloat("Gesture.Swipe.MinLength", 50.0f);
  leap.config().setFloat("Gesture.Swipe.MinVelocity", 50f);
  leap.config().setFloat("Gesture.Circle.MinRadius", 10.0f);
  leap.config().setFloat("Gesture.Circle.MinArc", .25f);
  leap.config().save();
}
...
void gestureControl(){
   ...
   CircleGesture circle = new CircleGesture(leap.frame().gesture(currentCircle));
   if(circle.isValid()){
     float turn = circle.progress();
     System.out.println("Turn: " + (turn-turns));
     if(turn - turns > 0.5) {
        turns += 0.5;
        String clockwiseness;
        if (circle.pointable().direction().angleTo(circle.normal()) <= Math.PI/2) {
               motrisGame.commandQueue.push(3); //clockwise
        }
        else
        {
     ...
     Gesture swipe = leap.frame().gesture(currentSwipe);
     if(swipe.isValid()){
       ...
       for(int g = 0; g < gestures.count(); g++){
          if(gestures.get(g).type() == Gesture.Type.TYPE_CIRCLE){
             currentCircle = gestures.get(g).id();
          } 
          if(gestures.get(g).type() == Gesture.Type.TYPE_SWIPE){
              SwipeGesture aSwipe = new SwipeGesture(gestures.get(g));
              Vector direction = aSwipe.direction();
             if(direction.angleTo(Vector.down()) < Math.PI/4){
                 currentSwipe = gestures.get(g).id();
             }
          } 
       }
     }
