...
void draw(){
  ...
  if(frame != lastFrame)
  {
    ...
    for(int g = 0; g < gesturesSince.count(); g++)
    {
        ...
        float duration;
        int pointableID = 0;
        if( gesturesSince.get(g).type() == Gesture.Type.TYPE_SCREEN_TAP)
        {
          ScreenTapGesture screenTap = new ScreenTapGesture(gesturesSince.get(g));
          normTapPoint = iBox.normalizePoint(screenTap.position(), true);
          duration = screenTap.duration();
          pointableID = screenTap.pointable().id();
        } else
        {
          KeyTapGesture keyTap = new KeyTapGesture(gesturesSince.get(g));
          normTapPoint = iBox.normalizePoint(keyTap.position(), true);  
          duration = keyTap.duration(); 
          pointableID = keyTap.pointable().id();       
        }
        ...
        } while( pastFrame.timestamp() < frame.timestamp() - duration);
        
        startPoints.add(iBox.normalizePoint(pastFrame.pointable(pointableID).stabilizedTipPosition(),true));
    }
  }
  lastFrame = frame;
...
void drawNormalizedPoint(Vector point, int pointColor)
{
       stroke(pointColor);
       pushMatrix();
       translate(point.getX() * width, height * (1 - point.getY()), point.getZ());
       sphere(1);
       popMatrix();  
}
