...
void draw() 
{
  ...
  if(finger.isValid())
  {
    ...
    float pixelX = normalizedPosition.getX() * width * gauss(normalizedPosition.getX());
    float pixelY = height - normalizedPosition.getY() * height * gauss(normalizedPosition.getY());
    cx = (int) pixelX - cw/2;
    cy = (int) pixelY - ch/2;
...
float gauss(float x)
{
    return a * (float)Math.exp(-((x - b * b)/(2 * c * c))) + d;
}
